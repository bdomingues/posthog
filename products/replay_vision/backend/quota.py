...
def compute_quota_snapshot(organization_id: UUID) -> QuotaSnapshot:
    ...
    enabled_scanners = ReplayScanner.objects.filter(
        team__organization_id=organization_id,
        enabled=True,
    ).aggregate(
        projected=Coalesce(Sum("estimated_monthly_observations"), Value(0)),
        count=Count("id"),
    )
    projected = enabled_scanners["projected"]
    enabled_scanner_count = enabled_scanners["count"]
