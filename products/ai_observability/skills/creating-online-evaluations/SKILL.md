...
description: >
  ...
  gating on provider keys and trial credits before an llm_judge eval, scoping which events fire via
...
separate job — run `exploring-ai-failures` to scope a use case, find failing traces, and produce a ranked
list of failure modes. Come back with one concrete, frequent failure in hand.

For the mechanics of _writing and iterating_ an evaluator (Hog source vs LLM-judge prompt, dry-running,
debugging a live eval), defer to `exploring-llm-evaluations`. For a worked example that streams results to
Slack, see `feature-usage-feed`.

## Tools

| Tool                                   | Purpose                                                            |
| -------------------------------------- | ------------------------------------------------------------------ |
| `posthog:llma-evaluation-config-get`   | Check trial credits and the team's active provider key (llm_judge) |
| `posthog:llma-provider-key-list`       | Find a usable (`ok` state) provider key to pin (llm_judge)         |
| `posthog:llma-evaluation-judge-models` | List valid provider+model combos and which are PostHog-funded      |
...
modes — then come back. With that list in hand, choose what to turn into an eval:

- **Most frequent, most painful first.** A handful of modes usually cover the majority of failures.
- **Fix the obvious first.** If a prompt tweak makes the failure go away, do that — evals earn their keep
  on failures that _persist_ after the easy fixes, not on ones a fix removes.
...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `hog`       | Structural / rule-based (JSON parses, length, regex, tool-call shape). Cheap, deterministic, **no key or credits needed.**              |
| `llm_judge` | Subjective / fuzzy (tone, factuality, on-topic). Costs an LLM call per run; needs AI data-processing approval + a key or trial credits. |
| `sentiment` | You want sentiment labels on user messages, not a pass/fail.                                                                            |

Reach for `hog` by default. `exploring-llm-evaluations` has the full Hog-vs-judge decision table and how
to write and dry-run each evaluator.
...
```json
posthog:llma-evaluation-config-get        // trial_evals_remaining, active_provider_key
posthog:llma-provider-key-list            // pick a key whose state == "ok"
posthog:llma-evaluation-judge-models      // { "provider": "openai" } → valid models + posthog_available
```

- **Trial credits** — `trial_evals_remaining > 0` and a model with `posthog_available: true`. Omit
  `provider_key_id`; PostHog funds it.
- **BYOK** — pin `model_configuration.provider_key_id` to an `ok`-state key. Required once trial credits
  run out or for non-allowlisted models.

If there are no trial credits and no `ok` key, stop and ask the user to add/validate a key in the UI — the
agent can't create keys.
...
`"model_configuration": { "provider": "openai", "model": "gpt-5-mini", "provider_key_id": "<uuid or omit for trial>" }`.
...
It now runs on every new matching `$ai_generation`. This isn't one-and-done: watch the results in
`exploring-llm-evaluations`, and when an eval's failures spike or a new mode appears, head back to
`exploring-ai-failures` to characterize it. To wire results into a Slack feed, see `feature-usage-feed`.
...
- **Fix the easy stuff before building an eval.** Reserve evals for failures that persist after prompt fixes.
- **`hog` first.** No key, no credits, no AI approval, deterministic. Reach for `llm_judge` only when the
...
- **Gate llm_judge before creating**, not after. A judge eval with no funded model errors on first run.
