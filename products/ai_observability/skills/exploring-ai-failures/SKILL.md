...
description: >
  ...
  ranked failure taxonomy. Hand off to creating-online-evaluations when a failure mode is worth catching
  automatically.
---
...
For each trace, note in plain language what went wrong. When a trace fails in a chain, record the _first_
thing that broke — the root failure usually causes the downstream symptoms, and fixing it clears them.
Group the notes into a few named failure modes ("ignores the date filter", "invents a policy", "drops the
second question"); an LLM can help cluster your notes, but review the groupings yourself. Keep reading
until new traces stop turning up new modes (tens of traces, not thousands — stop when it goes quiet).

## Step 4 — Rank and decide what to do

Rank the modes you found _by reading_, roughly by how often they showed up in your sample — a handful
usually dominate. The output is a short, ranked list of named failure modes, each with a representative
trace you actually opened. (A list assembled from error messages or metric counts you never read is the
loud subset, not this — go back to Step 3.) From there:

- **Fix the obvious ones** — a prompt tweak often removes a whole mode. Do that first.
- **Prioritize / file** — turn the ranked list into bugs or roadmap input.
- **Catch it automatically** — for a frequent mode that _persists_ after the easy fixes, turn it into an
  online eval. Hand off to `creating-online-evaluations` with the specific failure in hand.
...
- **Traces:** `https://app.posthog.com/ai-observability/traces` (filter to your use case)
- **Single trace:** see the URL conventions in `exploring-llm-traces`.
