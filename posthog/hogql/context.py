...
class HogQLContext:
    ...
    def add_access_control_warning(self, resource: str, count: int, message: str) -> None:
        from posthog.schema import AccessControlFilterWarning  # noqa: PLC0415

        self.access_control_warnings[resource] = AccessControlFilterWarning(
            resource=resource, count=count, message=message
        )
