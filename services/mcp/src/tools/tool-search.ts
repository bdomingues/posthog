/**
 * Shared tool-search logic for `exec search` and the tool-search debugger.
 *
 * Two strategies live here so they can't drift between the server runtime
 * (src/tools/exec.ts) and the standalone debugger (scripts/tool-search-debug.ts):
 *
